...
def launch_vms(size, cpu, ram):
    ...
    for i in range(size):
        ...
        subprocess.run(
            f"multipass launch -n {vm_name} --cpus {cpu} --memory {ram}M --cloud-init cloud-init.yaml",
            shell=True
        )
...
def update_etc_hosts(vm_ips):
    for name, ip in vm_ips.items():
        ...
        for other_name, other_ip in vm_ips.items():
            if name != other_name:
                echo_command = (
                    f"multipass exec {name} -- sudo bash -c "
                    f"\"echo '{other_ip} {other_name}' >>"
                )
...
def generate_ssh_keys(size):
    ...
    for i in range(size):
        ...
        public_key = subprocess.run(
            f"multipass exec {vm_name} -- cat /home/ubuntu/.ssh/id_rsa.pub",
            capture_output=True,
            text=True,
            shell=True
        )
...
def setup_nfs_common(size):
    for i in range(1, size):
        ...
        subprocess.run(
            f"{bash} -- sudo mount -t nfs manager:/home/ubuntu/cloud /home/ubuntu/cloud",
            shell=True
        )

        fstab_content = "manager:/home/ubuntu/cloud /home/ubuntu/cloud nfs"
        subprocess.run(
            f"{bash} -- sudo bash -c \"echo '{fstab_content}' >> /etc/fstab\"",
            shell=True
        )
...
def create_mpi_hosts(size):
    ...
    subprocess.run(
        "multipass exec manager -- touch /home/ubuntu/cloud/mpi_hosts",
        shell=True
    )
    ...
    for i in range(size):
        vm_name = "manager" if i == 0 else f"worker{i}"
        subprocess.run(
            f"multipass exec manager -- bash -c \"echo '{vm_name}' >> /home/ubuntu/cloud/mpi_hosts\"",
            shell=True
        )
